...
export default {
  ...
  onMouseMove: throttle((e) => {
    ...
    if (e.target.attrs.isOutPoint || e.target.attrs.isOutPointOut) {
      if (/^circle/.test(e.target.attrs.parent)) {
        group.find((point) => {
          if (point.attrs.id === e.target.attrs.parent) {
            point.attr('fill', themeColor);
          }
          return false;
        });
      } else {
      ...
      group.findAll((point) => point.attrs.isOutPoint).forEach((point) => {
